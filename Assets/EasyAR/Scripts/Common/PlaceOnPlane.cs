        ...
        if (!isObjectPlaced)
        {
            ...
            if (m_RaycastManager.Raycast(rayEmitPosition, s_Hits, TrackableType.PlaneWithinPolygon))
            {
                ...
                */
                ...
                for (int i = 0; i < InitialData.setObjectMaterials.Length; i++)
                {
                    InitialData.setObjectMaterials[i].shader = (Shader)Resources.Load("StandradShader", typeof(Shader));
                    InitialData.setObjectMaterials[i].color = new Color32(255, 255, 255, 255);
                }
            }
        }
        else
